...
def _fail_safe_check():
    ...
    # Check stop_event every move so Stop is responsive, not deferred
    # to the next focus-loss pause.
    se = getattr(p, "stop_event", None)
    if se is not None and se.is_set():
        restore_mouse_settings()
        raise PauseException("stop")
...
def _report_move(end_x, end_y, raw_path):
    """Feed target + planned path to the overlay in logical 1920x1080 coords."""
    if not tele.wants_target():
        return
    try:
        win = p.WINDOW
        comp = (win[2] / 1920.0) or 1.0
        if comp <= 0:
            comp = 1.0
        n = len(raw_path)
        step = max(1, n // 48)
        logical = [((float(px) - win[0]) / comp, (float(py) - win[1]) / comp)
                   for px, py in raw_path[::step]]
        tele.target((end_x - win[0]) / comp, (end_y - win[1]) / comp, logical)
    except Exception:
        pass


def _scale_move_times(times):
    """Scale timing by p.MOUSE_SPEED %. Clamped to [10, 500] so 0% can't freeze."""
    try:
        spd = float(getattr(p, "MOUSE_SPEED", 100) or 100)
    except (TypeError, ValueError):
        spd = 100.0
    spd = max(10.0, min(500.0, spd))
    if abs(spd - 100.0) < 1e-6:
        return times
    return np.asarray(times, dtype=float) * (100.0 / spd)
...
def moveTo(x, y, duration=0, delay=0.0, tsize=(5.0, 5.0), offset_x=0, offset_y=0, curve=0.8, n_sub=None, inertia=False):
    ...
    while True:
        ...
        times = _scale_move_times(traj["times"])

        if not reported:
            _report_move(end_x, end_y, raw_path)
            reported = True
