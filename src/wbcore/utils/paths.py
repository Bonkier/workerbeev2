...
def _runtime_base_path():
    ...
    # Nuitka standalone: assets sit next to the executable.
    if "__compiled__" in globals():
        exe_path = os.path.abspath(sys.executable)
        return os.path.dirname(exe_path)

    # Source tree: paths.py is at src/wbcore/utils/; project root is 3 up.
...
def _read_app_version(default="0.0.0"):
    # `version` (historical bundles) before `version.json` (repo format).
    for candidate in (VERSION_FILE, VERSION_FILE + ".json"):
        try:
...
PACKS = {
    ...
    'MidspringDream2'             : ((), (15,)),
    'The_BE'                      : ((), (3, 4, 5)),
    # Collab (Normal 4-5F, Hard 4-10F). Hard 4-10 maps to the floor-5 pool
    # via format_lvl's 6-10 -> 5.
    'PilgrimageofCompassion'      : ((4, 5), (4, 5)),
