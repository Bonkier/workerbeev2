# Linux (X11) port. Deps: python-xlib, mss.
...
from .profiles import get_macro_profile, maybe_rhythm_jitter, randomize_with_profile
from .movement.builder import build_trajectory
from .movement.inertia import get_inherited_velocity, update_inertia
from .movement.pointer_gain import update_pointer_scale, execute_trajectory
...
def _get_window_title(win):
    """Title via _NET_WM_NAME, falling back to WM_NAME."""
    try:
...
def _find_window_by_name(name):
    """Find a _NET_CLIENT_LIST entry whose title contains `name`."""
    try:
...
def center(target=None):
    """Center of a window title, region (l,t,w,h), or primary screen."""
...
def get_virtual_screen_bounds():
    """(min_x, min_y, max_x, max_y) of the virtual desktop; coords may be
    negative. Mirrors Windows' SM_XVIRTUALSCREEN / SM_CXVIRTUALSCREEN."""
...
def screenshot(imageFilename=None, region=None):
    """mss capture. region=(x,y,w,h). Returns BGR ndarray for cv2."""
    with mss.mss() as sct:
        if region:
...
def _pick_device_paths():
    ...
    for path in kbd_data:
        try:
            ...
            # 2. Keyboard: ESC + lots of keys is a strong signal.
            if e.KEY_ESC in key_caps and len(key_caps) > 50:
                ...
                if phys.endswith("/input0"):
                        score += 50  # primary typing endpoint
                elif phys.endswith("/input1") or phys.endswith("/input2"):
                        score -= 50  # media/NKRO endpoints
...
def _init_uinput_devices():
    """Initialize uinput devices once; publish to module globals."""
...
def _prewarm_uinput_async():
    """Kick off uinput init in a thread so startup isn't blocked."""
...
# First input call blocks only if init is still in flight.
_prewarm_uinput_async()
...
def _sample_hold_seconds(kind, profile=None):
    """Sample hold duration from median/IQR; clamp to profile bounds."""
    if kind == "click":
...
def _emit_rel_open_loop(dev, dx, dy):
    """Emit a REL step; round-carry preserves sub-pixel intent."""
    ix = int(round(dx))
    iy = int(round(dy))
    if ix == 0 and iy == 0:
        return
...
def scroll(clicks, x=None, y=None):
    ...
    for _ in range(count):
        ...
        # Hi-res wheel: 120 units per detent.
...
def _get_x_keycode(keysym):
    """Cached XKB-layout-aware keysym -> X keycode."""
...
# ASCII -> X keysym for layout-aware lookup.
_ASCII_TO_XK = {}
...
def _key_to_ecode(key):
    """Key name -> evdev scancode via layout-aware X11 mapping."""
    key_lower = key.lower()
...
def set_window():
    """Center a 16:9 region inside the LIMBUS_NAME client area (mirrors Windows)."""
